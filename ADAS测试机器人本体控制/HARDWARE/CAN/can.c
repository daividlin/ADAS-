...
u8 CAN1_Mode_Init(u8 tsjw, u8 tbs2, u8 tbs1, u16 brp, u8 mode)
{
	...
	CAN1->MCR |= 1 << 6;//0<<6;		//软件自动离线管理
	CAN1->MCR |= 0 << 5;		//睡眠模式通过软件唤醒(清除CAN1->MCR的SLEEP位)
	CAN1->MCR |= 0 << 4;//1<<4;		//禁止报文自动传送
	CAN1->MCR |= 0 << 3;		//报文不锁定,新的覆盖旧的
	CAN1->MCR |= 0 << 2;		//优先级由报文标识符决定
	CAN1->BTR = 0x00000000;	//清除原来的设置.
	CAN1->BTR |= 0 << 30;//mode<<30;	//模式设置 0,普通模式;1,回环模式;
	CAN1->BTR |= 0 << 24;//tsjw<<24; 	//重新同步跳跃宽度(Tsjw)为tsjw+1个时间单位
	CAN1->BTR |= 2 << 20;//tbs2<<20; 	//Tbs2=tbs2+1个时间单位
	CAN1->BTR |= 1 << 16;//tbs1<<16;	//Tbs1=tbs1+1个时间单位
	CAN1->BTR |= 13 << 0;//brp<<0;  	//分频系数(Fdiv)为brp+1
...
void CAN1_RX0_IRQHandler(void)
{
	...
	{
	case 0x580:
		switch (((int)rxbuf[1]) | ((int)rxbuf[2] << 8))
		{
		case 0x6061:
			gHalData.WheelHal[(id & 0x7f) - 1].ObDict[ACTUALTYPE].Value[0] = rxbuf[4];
			break;
		case 0x6041:
			gHalData.WheelHal[(id & 0x7f) - 1].ObDict[STATUSWORD].Value[0] = rxbuf[4];
			gHalData.WheelHal[(id & 0x7f) - 1].ObDict[STATUSWORD].Value[1] = rxbuf[5];
			break;
		}
		break;
	case 0x180:
		gHalData.WheelHal[(id & 0x7f) - 1].ObDict[STATUSWORD].Value[0] = rxbuf[0];
		gHalData.WheelHal[(id & 0x7f) - 1].ObDict[STATUSWORD].Value[1] = rxbuf[1];
		//			gHalData.WheelHal[(id & 0x7f) - 1].ObDict[ACTUALPOS].Value[0]	= rxbuf[2];
		//			gHalData.WheelHal[(id & 0x7f) - 1].ObDict[ACTUALPOS].Value[1]	= rxbuf[3];
		//			gHalData.WheelHal[(id & 0x7f) - 1].ObDict[ACTUALPOS].Value[2]	= rxbuf[4];
		//			gHalData.WheelHal[(id & 0x7f) - 1].ObDict[ACTUALPOS].Value[3]	= rxbuf[5];
		gHalData.WheelHal[(id & 0x7f) - 1].ObDict[ACTUALVEL].Value[0] = rxbuf[2];
		gHalData.WheelHal[(id & 0x7f) - 1].ObDict[ACTUALVEL].Value[1] = rxbuf[3];
		gHalData.WheelHal[(id & 0x7f) - 1].ObDict[ACTUALVEL].Value[2] = rxbuf[4];
		gHalData.WheelHal[(id & 0x7f) - 1].ObDict[ACTUALVEL].Value[3] = rxbuf[5];
		break;
	default:
		break;
	}
...
u8 CAN1_Send_Msg(u8* msg, u8 len)
{
	...
	//返回值:0~3,邮箱编号.0XFF,无有效邮箱.
	//u8 CAN1_Tx_Msg(u32 id,u8 ide,u8 rtr,u8 len,u8 *dat);	
	mbox = CAN1_Tx_Msg(0x0211, 0, 0, len, msg);
	while ((CAN1_Tx_Staus(mbox) != 0X07) && (i < 0XFFF))i++;//等待发送结束
	if (i >= 0XFFF)return 1;							//发送失败?
	return 0;										//发送成功;
}
u8 CAN1_Send_Msg_id(u32 id, u8* msg, u8 len)
{
	u8 mbox;
	u16 i = 0;
